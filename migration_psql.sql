-- Aplicar migraciones F29 Contapyme
-- Conexión: postgresql://postgres.yttdnmokivtayeunlvlk:matigol1234@aws-0-us-east-1.pooler.supabase.com:6543/postgres

-- ==========================================
-- CONFIGURACIÓN SUPABASE PARA CONTAPYME F29
-- Aplicación directa sin dependencias
-- ==========================================

-- Crear tabla principal de formularios F29 (simplificada)
...
CREATE TABLE IF NOT EXISTS f29_insights_notifications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_f29_confidence ON f29_forms(confidence_score DESC);
CREATE INDEX IF NOT EXISTS idx_f29_upload_date ON f29_forms(upload_date DESC);

-- Índices para métricas (para análisis rápidos)
CREATE INDEX IF NOT EXISTS idx_f29_metrics_sales ON f29_forms(company_id, ventas_netas DESC);
CREATE INDEX IF NOT EXISTS idx_f29_metrics_margin ON f29_forms(company_id, margen_bruto DESC);
CREATE INDEX IF NOT EXISTS idx_f29_metrics_iva ON f29_forms(company_id, iva_determinado);
...
CREATE INDEX IF NOT EXISTS idx_f29_analysis_expires ON f29_comparative_analysis(expires_at);

-- Índices para notificaciones
CREATE INDEX IF NOT EXISTS idx_f29_notifications_user ON f29_insights_notifications(user_id, is_read);
CREATE INDEX IF NOT EXISTS idx_f29_notifications_company ON f29_insights_notifications(company_id, created_at DESC);
...
DROP TRIGGER IF EXISTS update_f29_notifications_updated_at ON f29_insights_notifications;
...
ALTER TABLE f29_forms DISABLE ROW LEVEL SECURITY;
ALTER TABLE f29_comparative_analysis DISABLE ROW LEVEL SECURITY;
ALTER TABLE f29_insights_notifications DISABLE ROW LEVEL SECURITY;
...
INSERT INTO f29_forms (
  ...
  '202412',
  'test_setup.pdf',
  '{"codigo563": 1000000, "codigo538": 190000, "codigo511": 150000, "codigo062": 2000, "codigo077": 5000}',
  95,
  'validated'
) ON CONFLICT (company_id, period) DO NOTHING;

-- Mensaje de confirmación
SELECT 'Setup Supabase completado exitosamente!' as status,
       COUNT(*) as tablas_creadas
FROM information_schema.tables 
WHERE table_name IN ('f29_forms', 'f29_comparative_analysis', 'f29_insights_notifications');