...
/**
 * DELETE /api/rcv/[type]
 * Elimina un ledger y sus documentos asociados
 */
export async function DELETE(
  ...
  try {
    ...
    if (!ledgerId) {
      return NextResponse.json(
        { success: false, error: 'ledger_id es requerido' },
        { status: 400 }
      );
    }
    ...
    console.log(`🗑️ RCV DELETE - Tipo: ${rcvType}, Ledger ID: ${ledgerId}`);
    ...
    return NextResponse.json({
      success: true,
      message: `${rcvType === 'purchase' ? 'Compras' : 'Ventas'} eliminadas exitosamente`
    });
